...
export default {
    ...
    methods: {
        ...
        obtenerEntradaBreakValida(record) {
            const rawBreak = record.attendanceDetailInfo?.recordTime?.[0]?.endTime;
            const rawSalida = record.attendanceBaseInfo?.endTime;

            if (!rawBreak) return null;

            // Convertir a objeto Date para evaluar la hora fácilmente
            const fechaBreak = new Date(rawBreak);
            const hora = fechaBreak.getHours(); // Retorna hora en formato 24h (0 - 23)

            // Regla: Si es de las 15:00 (3:00 p.m.) en adelante O si coincide con la salida final
            if (hora >= 15 || (rawSalida && rawBreak === rawSalida)) {
                return null;
            }
